...
- **Two image namespaces, chosen by the `src` shape.** A **bare filename** (`![alt](hero.png)` / `<img src="hero.png">`) is **hd-managed** — it resolves to the doc's sidecar folder `<workspace>/.hd/<id>/hero.png` (the paste/asset target). A **relative path** (`./shot.png`, `../out/gen.png`) is **in-place** — it resolves relative to the doc's own folder and references a file that already exists on disk, with **no copy** into `.hd/<id>/`. Use in-place for build/generation artifacts already versioned in the repo (e.g. an agent's `.hd` report over generated media); use managed for pasted/captured images. In-place files must live inside the workspace; publishing still copies them to the destination. A doc with **managed** media **must** have an `id` in frontmatter, and you must create `.hd/<id>/` before placing files. Inline `<svg>` is exempt (it lives in the body, not a file).
